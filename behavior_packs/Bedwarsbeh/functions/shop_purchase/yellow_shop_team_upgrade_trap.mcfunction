...
execute @a[tag=upgrade_yellow_trap] ~~~ tellraw @s { "rawtext" : [ {"translate":"text.tellraw.ingameinfo.purchase_team_upgrade_trap"} ] }
execute @a[tag=upgrade_yellow_trap] ~~~ tellraw @a[scores={"分队"=3}] { "rawtext" : [ { "text" : "§e " } ,{ "selector" :  "@a[tag=want_upgrade_trap_yellow]"},{"translate":"text.tellraw.ingameinfo.team_member_purchase_team_upgrade_trap"}] }
